...
//        .reset(sysreset), // input reset
	...
	//Load operands
	integer i, j;
	integer position_tmp[N_INPUTS-1 : 0];
	reg [N_INPUTS-1 : 0] position_tmp_transpose [31:0];
...
// 	end
    ...
    // OR-reduction to determine final position
 	always @(posedge sysclk) begin
        for( i = 0; i < 32; i++) begin
            position[i] <= | (position_tmp_transpose[i]);
        end
 	end
