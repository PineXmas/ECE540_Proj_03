...
//	wire		[CNTR_WIDTH-1:0]	top_cnt = SIMULATE ? SIMULATE_FREQUENCY_CNT : ((CLK_FREQUENCY_HZ / DEBOUNCE_FREQUENCY_HZ) - 1);
    ...
    // [Problem #1] fix
    reg		[CNTR_WIDTH-1:0]	top_cnt;
    reg is_top_cnt_reached;
    always @(posedge clk)
    begin
        top_cnt <= SIMULATE ? SIMULATE_FREQUENCY_CNT : ((CLK_FREQUENCY_HZ / DEBOUNCE_FREQUENCY_HZ) - 1);
    end
    always @(posedge clk)
    begin
        is_top_cnt_reached <= db_count == top_cnt;
    end
